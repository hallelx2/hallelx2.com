...
page(
    ...
    [
     sec("01", "The shape, and why it is not one service", None,
         ...
         "</div></div>"),
     sec("02", "Modular monolith, not microservices", "Two services were carved out. Everything else stayed put, and that was the decision.",
         fig("The shape I keep choosing",
             "Microservices distribute your monolith's problems and add a network to them.",
             chain([node("one deployable core", "commerce domain", "on"),
                    node("carve out only what differs", "agent, image-gen", "on"),
                    node("shared contracts", "compile-time, not runtime", "on"),
                    node("Kafka only at the boundary", "events in-process otherwise")]),
             "Mercala could have been eight services. It is a core plus two, and the two came out for reasons I can name: the agent has a different scaling shape and a different failure mode, and image generation is slow, bursty work that should not sit in a request thread. <b>Everything else stayed in the monolith because nothing about it wanted a network in the middle.</b> The cost of a service is not the code &mdash; it is a deployment, a version, a retry policy, a tracing story and a class of failure that did not exist when it was a method call. A module boundary gives you most of the isolation for none of that, and it is <b>reversible</b>: promoting a module to a service later is mechanical, while merging two services back is not.",
             [("1", "core deployable"), ("2", "carved out, for named reasons"), ("0", "networks added between things that agree")])),
     sec("03", "Multi-tenancy, three layers deep", "Never weaken a layer without a test proving the others still hold.",
     ...
     sec("04", "What Spring is actually carrying", "Each one is there for a named failure.",
         does([
     ...
     sec("05", "Search, without the second cluster", "The decision I would defend hardest on this project.",
     ...
     sec("06", "Deployment, and the keys that do not exist",
...
page(
    ...
    [
     ...
     sec("05", "One repository: web, mobile and the backend", "Every TypeScript product I build has the same shape, and the shape is the point.",
         fig("apps/ and packages/",
             "The web app, the phone app and the server that feeds them share one set of types.",
             chain([node("apps/web", "Next.js"),
                    node("apps/mobile", "Expo"),
                    node("apps/server", "Hono or Bun"),
                    node("packages/*", "schemas, client, tokens, db", "on")]),
             "Coursified is five apps &mdash; web, mobile, server, relay, fetcher &mdash; over <b>thirteen packages</b>. Voxtar is six apps over three packages plus a Python service. Tether is two apps over one. The counts differ; the shape never does: <b>deployables in <code>apps/</code>, anything two of them share in <code>packages/</code></b>. The reason is narrow and it is about when you find out you were wrong. A shared type in a package means changing it breaks the build of every app that imports it, in CI, before anything ships. The same type copied into a web repo and a mobile repo means you find out from a user.",
             [("5", "Coursified apps"), ("13", "packages they share"), ("1", "place a shared type is declared")])),
     sec("06", "What earns its own package", "Not everything. The test is whether two apps disagree if it drifts.",
         does([
           ("Contracts", "<code>schemas</code> and <code>api-client</code>. The server validates with the same schema the client sends against, so the wire format has one definition rather than two that agree today."),
           ("Tokens", "Colour and type, read by web and mobile alike — which is the only reason the phone build still looks like the product."),
           ("The database layer", "<code>db</code> as a package means the schema, the migrations and the query helpers travel together, and nothing reaches past them into raw SQL by accident."),
           ("Auth", "One implementation of who-you-are, because two is how a mobile app ends up trusting a token the web app would have rejected."),
           ("What does not", "A helper used once. A package with one consumer is a directory with extra ceremony, and I have deleted more of those than I have kept."),
         ])),
     sec("07", "The part people get wrong", "A monorepo does not make versions agree. It makes disagreement visible.",
         does([
           ("Pin what must be singular", "React, React Native and CodeMirror are pinned in root <code>overrides</code>. Workspaces resolve compatible-but-different versions perfectly happily, and for these that is a bug."),
           ("Coursified proved it", "Two copies of <code>@codemirror/state</code> are two <b>nominal</b> types, because its classes carry private fields. Every <code>EditorView</code> passed between them failed to typecheck. That broke production on 16 September 2026."),
           ("The pin lives in package.json", "Not the lockfile — because the incident was Vercel's bun failing to parse the lockfile and resolving fresh. A fix that only exists in a lockfile is a fix that assumes the lockfile is read."),
           ("Write the reason next to it", "The <code>overrides</code> block carries a paragraph explaining why. A pin without its reason is the next person's bug, and the next person is usually me."),
         ])),
     sec("08", "The repository", None,
