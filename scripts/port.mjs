...
for (const f of ["nav.css", "product.css"]) {
...
const ASSETS = [
  ...
  "img/hero-campus.png", "img/hero-clouds.png", "img/hero-shelves.png", "img/og-card.png",
...
// variance point 3: the end CTA label ("Get in touch" / "Book a cohort")
navJsx = navJsx.replaceAll(`<span>{"Get in touch"}</span>`, `<span>{cta}</span>`);
if (!navJsx.includes("{cta}") || !navJsx.includes('active === "about"') || !navJsx.includes('active === "training"'))
